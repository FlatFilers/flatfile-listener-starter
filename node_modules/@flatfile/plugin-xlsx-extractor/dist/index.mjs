...
var AbstractExtractor = class {
   ...
   */
  getFileBufferFromApi() {
    return __async(this, null, function* () {
      const file = yield this.api.downloadFile({ fileId: this.fileId });
  ...
  startJob() {
    return __async(this, null, function* () {
      try {
        const res = yield this.api.createJob({
          jobConfig: {
            type: "file",
            operation: "extract",
            status: "executing",
  ...
  completeJob(job) {
    return __async(this, null, function* () {
      try {
        const res = yield this.api.updateJob({
          jobId: job.id,
          jobUpdate: {
            status: "complete"
          }
   ...
   */
  createWorkbook(file, workbookCapture) {
    return __async(this, null, function* () {
      var _a;
      ...
      try {
        const workbook = yield this.api.addWorkbook({ workbookConfig });
        if (!workbook || !workbook.data) {
          throw new Error(
            `Unable to create workbook: ${JSON.stringify(workbook)}`
          );
        }
        yield this.api.updateFileById({
          fileId: file.id,
          fileConfig: { workbookId: (_a = workbook.data) == null ? void 0 : _a.id }
        });
  ...
  getSheetConfig(name, { headers, required, descriptions }) {
    return {
      ...
      fields: headers.map((key) => ({
        key,
        label: key,
        description: (descriptions == null ? void 0 : descriptions[key]) || void 0,
        type: "string",
        constraints: (required == null ? void 0 : required[key]) ? [{ type: "required" }] : void 0
      }))
    };
  }
};
...
var ExcelExtractor = class extends AbstractExtractor {
  constructor(event, options) {
    ...
    this._options = __spreadValues({ rawNumbers: false }, options);
  }
  /**
   ...
   */
  convertSheet(sheet) {
    let rows = utils.sheet_to_json(sheet, {
      header: "A",
      defval: null,
    ...
    if (colMap) {
      const headers = mapValues(colMap, (val) => val == null ? void 0 : val.replace("*", ""));
      const required = mapValues(colMap, (val) => val == null ? void 0 : val.includes("*"));
  ...
  runExtraction() {
    return __async(this, null, function* () {
      try {
        const { data: file } = yield this.api.getFile({ fileId: this.fileId });
        if (file.ext !== "xlsx") {
          return false;
        }
        const job = yield this.startJob();
        const buffer = yield this.getFileBufferFromApi();
        const capture = this.parseBuffer(buffer);
        const workbook = yield this.createWorkbook(file, capture);
        if (!(workbook == null ? void 0 : workbook.sheets))
          return false;
        for (const sheet of workbook.sheets) {
          ...
          yield this.api.addRecords({
            sheetId: sheet.id,
            recordsData
          });
        }
        yield this.completeJob(job);
        return true;
      } catch (e) {
        return false;
      }
